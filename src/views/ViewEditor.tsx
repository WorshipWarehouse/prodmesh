...
function WidgetInspector({ placement, onChange }: { placement: ViewPlacement | null; onChange: (id: string, patch: Record<string, unknown>) => void }) {
  ...
  if (placement && obs) return <ObsHealthInspector placement={placement} onChange={onChange} />;
...
function ObsHealthInspector({ placement, onChange }: { placement: ViewPlacement; onChange: (id: string, patch: Record<string, unknown>) => void }) {
  return <aside className="widgetinspector">
    ...
    <label className="widgetinspector__check">
      <input type="checkbox" checked={placement.config.obsPreview ?? true} onChange={(event) => onChange(placement.id, { obsPreview: event.target.checked })} />
      Show program preview
    </label>
    <small>Uses the optional preview image URL configured for this room in Campus setup.</small>
    <label className="widgetinspector__check">
      <input type="checkbox" checked={Boolean(placement.config.obsDetails)} onChange={(event) => onChange(placement.id, { obsDetails: event.target.checked })} />
      Show system details
    </label>
    <small>Shows OBS CPU usage, available disk space, and program-source count.</small>
  </aside>;
}
